...
更多文档：[Claude 指南](./CLAUDE.zh.md) | [Cursor 使用](./CURSOR.zh.md) | [示例](./EXAMPLES.zh.md)。每个页面顶部都有语言切换链接。
...
> "它们非常喜欢把代码和 API 复杂化，堆砌抽象概念，不清理死代码……明明 100 行可以完成的事情，却实现成 1000 行的臃肿架构。"
...
该插件同时包含英文 `karpathy-guidelines` skill 和中文 `karpathy-guidelines-zh` skill。
...
这些指南倾向于**谨慎优先，速度其次**。对于琐碎的任务（简单的拼写错误修复、显而易见的一行修改），请自行判断 —— 无需每个改动都走完整的严谨流程。

目标是减少非琐碎工作中的高代价错误，同时保持简单任务的速度。
